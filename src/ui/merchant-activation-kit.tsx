...
export function MerchantActivationKit({ merchant }: { merchant: Merchant }) {
  const experiencePath = `/${merchant.slug}`;
  const trackedPath = `/q/${merchant.slug}`;
  const [copied, setCopied] = useState(false);

  async function copyLink() {
    try {
      const publicUrl = new URL(trackedPath, window.location.origin).href;
        ...
        <div className="activation-content">
          <div className="activation-heading">
            <div>
              ...
              <p>Usá el QR o el enlace rastreable para llevar clientes a la experiencia pública de {merchant.theme.shortName}.</p>
            </div>
          ...
          <section className="activation-grid">
            <article className="activation-panel activation-link-panel">
              <p className="eyebrow">01 · Enlace rastreable</p>
              <h2>La URL oficial para QR y difusión.</h2>
              <div className="activation-url" data-testid="activation-public-url">
                <span>{trackedPath}</span>
                <button type="button" onClick={copyLink} data-testid="copy-activation-link">{copied ? "Copiado ✓" : "Copiar enlace"}</button>
              </div>
              <p className="activation-help">Este enlace registra la entrada antes de abrir la experiencia. Usalo en QR, WhatsApp, Google Business o piezas digitales. Para revisar sin sumar escaneos, usá “Ver experiencia”.</p>
            </article>
          ...
          <section className="activation-poster-section">
            ...
            <div className="activation-poster" data-testid="activation-poster">
              ...
              <div className="poster-copy">
                <p>TENEMOS UN REGALO<br />PARA VOS</p>
          ...
          <section className="activation-tips">
            ...
            <article><strong>WhatsApp</strong><span>Usá el enlace rastreable cuando quieras medir también esas entradas.</span></article>
          </section>
        </div>
